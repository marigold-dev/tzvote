...
function timeoutAfter(timeoutMillisec) {
    return function inner(source) {
        return new rxjs_1.BehaviorSubject(null).pipe(operators_1.timeoutWith(timeoutMillisec, rxjs_1.throwError(new Error('Confirmation polling timed out'))), operators_1.mergeMap(() => source));
    };
}
exports.timeoutAfter = timeoutAfter;
const createNewPollingBasedHeadObservable = (sharedHeadOb, context, _scheduler) => {
    return sharedHeadOb.pipe(timeoutAfter(context.config.confirmationPollingTimeoutSecond * 1000), operators_1.publishReplay(1), operators_1.refCount());
};
