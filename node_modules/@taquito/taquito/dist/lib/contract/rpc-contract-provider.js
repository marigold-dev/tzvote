...
class RpcContractProvider extends operation_emitter_1.OperationEmitter {
    ...
    transferTicket(params) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            if (utils_2.validateContractAddress(params.destination) !== utils_2.ValidationResult.VALID) {
                throw new utils_2.InvalidContractAddressError(params.destination);
            }
            if (params.source && utils_2.validateAddress(params.source) !== utils_2.ValidationResult.VALID) {
                throw new utils_2.InvalidAddressError(params.source);
            }
            ...
            const operation = yield prepare_1.createTransferTicketOperation(Object.assign(Object.assign({}, params), estimate));
            const source = (_a = params.source) !== null && _a !== void 0 ? _a : publicKeyHash;
            const ops = yield this.addRevealOperationIfNeeded(operation, publicKeyHash);
            const prepared = yield this.prepareOperation({ operation: ops, source: params.source });
            const opBytes = yield this.forge(prepared);
            const { hash, context, forgedBytes, opResponse } = yield this.signAndInject(opBytes);
            return new transfer_ticket_operation_1.TransferTicketOperation(hash, operation, source, forgedBytes, opResponse, context);
        });
    }
    /**
     *
    ...
    increasePaidStorage(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const operation = yield prepare_1.createIncreasePaidStorageOperation(Object.assign(Object.assign({}, params), estimate));
            const ops = yield this.addRevealOperationIfNeeded(operation, publicKeyHash);
            const prepared = yield this.prepareOperation({ operation: ops, source: publicKeyHash });
            const opBytes = yield this.forge(prepared);
            const { hash, context, forgedBytes, opResponse } = yield this.signAndInject(opBytes);
            return new increase_paid_storage_operation_1.IncreasePaidStorageOperation(hash, operation, publicKeyHash, forgedBytes, opResponse, context);
        });
    }
    /**
     *
    ...
    drainDelegate(params) {
        return __awaiter(this, void 0, void 0, function* () {
            const operation = yield prepare_1.createDrainDelegateOperation(Object.assign({}, params));
            const prepared = yield this.prepareOperation({ operation: operation });
            const opBytes = yield this.forge(prepared);
            const { hash, context, forgedBytes, opResponse } = yield this.signAndInject(opBytes);
            return new drain_delegate_operation_1.DrainDelegateOperation(hash, operation, forgedBytes, opResponse, context);
        });
    }
    /**
     *
    ...
    ballot(params) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const source = (_a = params.source) !== null && _a !== void 0 ? _a : publicKeyHash;
            const operation = yield prepare_1.createBallotOperation(Object.assign(Object.assign({}, params), { source }));
            const prepared = yield this.prepareOperation({ operation, source });
            const opBytes = yield this.forge(prepared);
            const { hash, context, forgedBytes, opResponse } = yield this.signAndInject(opBytes);
            return new ballot_operation_1.BallotOperation(hash, operation, publicKeyHash, forgedBytes, opResponse, context);
        });
    }
    /**
     *
    ...
    proposals(params) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const source = (_a = params.source) !== null && _a !== void 0 ? _a : publicKeyHash;
            const operation = yield prepare_1.createProposalsOperation(Object.assign(Object.assign({}, params), { source }));
            const prepared = yield this.prepareOperation({ operation, source });
            const opBytes = yield this.forge(prepared);
            const { hash, context, forgedBytes, opResponse } = yield this.signAndInject(opBytes);
            return new proposals_operation_1.ProposalsOperation(hash, operation, publicKeyHash, forgedBytes, opResponse, context);
        });
    }
    /**
     *
    ...
    updateConsensusKey(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const operation = yield prepare_1.createUpdateConsensusKeyOperation(Object.assign(Object.assign({}, params), estimate));
            const ops = yield this.addRevealOperationIfNeeded(operation, publicKeyHash);
            const prepared = yield this.prepareOperation({ operation: ops, source: params.source });
            const opBytes = yield this.forge(prepared);
            const { hash, context, forgedBytes, opResponse } = yield this.signAndInject(opBytes);
            return new update_consensus_key_operation_1.UpdateConsensusKeyOperation(hash, operation, publicKeyHash, forgedBytes, opResponse, context);
        });
    }
