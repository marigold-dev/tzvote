...
const scriptEncoderProto14 = (script) => {
    const code = exports.valueEncoderProto14(script.code);
    const storage = exports.valueEncoderProto14(script.storage);
...
const scriptDecoderProto14 = (value) => {
    ...
    return {
        code: exports.valueDecoderProto14(new uint8array_consumer_1.Uint8ArrayConsumer(code)),
        storage: exports.valueDecoderProto14(new uint8array_consumer_1.Uint8ArrayConsumer(storage)),
    };
};
exports.scriptDecoderProto14 = scriptDecoderProto14;
const valueEncoderProto14 = (value) => {
    if (Array.isArray(value)) {
        const encoded = value.map((x) => exports.valueEncoderProto14(x)).join('');
        const len = encoded.length / 2;
        return `02${utils_1.pad(len)}${encoded}`;
    }
    else if (codec_1.isPrim(value)) {
        return exports.primEncoderProto14(value);
    }
...
const valueDecoderProto14 = (value) => {
    ...
    switch (preamble[0]) {
        ...
        case 0x02: {
            ...
            const results = [];
            while (val.length() > 0) {
                results.push(exports.valueDecoderProto14(val));
            }
            return results;
        }
        default:
            return exports.primDecoderProto14(value, preamble);
    }
};
...
const primEncoderProto14 = (value) => {
    ...
    const op = constants_proto14_1.opMappingReverseProto14[value.prim];
    let encodedArgs = (value.args || []).map((arg) => exports.valueEncoderProto14(arg)).join('');
...
const primDecoderProto14 = (value, preamble) => {
    ...
    if (constants_proto14_1.opMappingProto14[op] === 'view') {
        if (argsCount != 0) {
            return exports.primViewDecoderProto14(value, result);
        }
        else {
            return result;
        }
    ...
    const args = new Array(argsCount).fill(0).map(() => exports.valueDecoderProto14(value));
    if (constants_proto14_1.opMappingProto14[op] === 'LAMBDA') {
        value.consume(4);
    }
    if (combPairArgs) {
...
const primViewDecoderProto14 = (value, result) => {
    value.consume(4);
    result['args'] = new Array(4).fill(0).map(() => exports.valueDecoderProto14(value));
    value.consume(4);
    return result;
};
...
const decodeCombPair = (val) => {
    ...
    while (array.length() > 0) {
        args.push(exports.valueDecoderProto14(array));
    }
    return args;
};
