...
var sha224_1 = require("@stablelib/sha224");
...
describe("pbkdf2.deriveKey (SHA256)", function () {
    it("should produce correct keys for test vectors", function () {
        testVectors256.forEach(function (v, i) {
            var password = base64_1.decode(v[0]);
            var salt = base64_1.decode(v[1]);
            var length = base64_1.decodedLength(v[2]);
            var iterations = 128 - i + 2;
            var dk = pbkdf2_1.deriveKey(sha256_1.SHA256, password, salt, iterations, length);
            expect(base64_1.encode(dk)).toBe(v[2]);
        });
    });
});
describe("pbkdf2.deriveKey (SHA224)", function () {
    it("should produce correct keys for test vectors", function () {
        testVectors224.forEach(function (v, i) {
            var password = base64_1.decode(v[0]);
            var salt = base64_1.decode(v[1]);
            var length = base64_1.decodedLength(v[2]);
            var iterations = 128 - i + 2;
            var dk = pbkdf2_1.deriveKey(sha224_1.SHA224, password, salt, iterations, length);
            expect(base64_1.encode(dk)).toBe(v[2]);
        });
    });
});
describe("pbkdf2.deriveKey (SHA512)", function () {
    it("should produce correct keys for test vectors", function () {
        testVectors512.forEach(function (v, i) {
            var password = base64_1.decode(v[0]);
            var salt = base64_1.decode(v[1]);
            var length = base64_1.decodedLength(v[2]);
            var iterations = 128 - i + 2;
            var dk = pbkdf2_1.deriveKey(sha512_1.SHA512, password, salt, iterations, length);
            expect(base64_1.encode(dk)).toBe(v[2]);
        });
    });
});
//# sourceMappingURL=pbkdf2.test.js.map