...
const openAlert = async (config) => {
    ...
    if (!isOpen()) {
        ...
        const extensionsUpdatedFn = async (event) => {
        ...
        const createWalletList = () => {
            const wallets = [
                ...desktopList
                    ...
                    // is Infinity Wallet.
                    .filter((wallet) => !availableExtensions.some((extWallet) => wallet.name === extWallet.name))
                    .map((wallet) => {
                    return {
                        id: wallet.key,
                        key: wallet.key,
        ...
        dispose = render(() => (<div class={`theme__${colorMode}`}>
          {config.pairingPayload && (<Alert loading={isLoading()} open={isOpen()} showMore={showMoreContent()} content={<div>
                  {currentInfo() === 'install' && (<div style={currentInfo() === 'install' || currentInfo() === 'qr'
                            ? {
                                opacity: 1,
                                height: 'unset',
                                overflow: 'unset',
                  ...
                  {currentInfo() === 'qr' && (<div style={currentInfo() === 'install' || currentInfo() === 'qr'
                            ? {
                                opacity: 1,
                                height: 'unset',
                                overflow: 'unset',
                      ...
                      <PairOther walletList={walletList()} onClickLearnMore={handleClickLearnMore} p2pPayload={p2pPayload} wcPayload={wcPayload}></PairOther>
                    </div>)}
                  <div style={currentInfo() === 'wallets'
                    ...
                    <Wallets disabled={isLoading()} wallets={walletList().slice(-(walletList().length - (isMobile() ? 3 : 4)))} isMobile={isMobile()} onClickWallet={handleClickWallet} onClickOther={handleClickOther}/>
                  </div>
                  <div style={currentInfo() === 'help'
                    ...
                    <Info iconBadge icon={<svg fill="none" stroke-width="2" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24" height="1em" width="1em" style="overflow: visible;" color="white">
                        ...
                        </svg>} title="Not sure where to start?" description="If you are new to the Web3, we recommend that you start by creating a Kukai wallet. Kukai is a fast way of creating your first wallet using your preferred social account."/>
                  </div>
                  <div style={currentInfo() !== 'install' &&
                    ...
                    <TopWallets disabled={isLoading()} wallets={isMobile() ? walletList().slice(0, 3) : walletList().slice(0, 4)} isMobile={isMobile()} onClickWallet={handleClickWallet} onClickLearnMore={handleClickLearnMore} otherWallets={isMobile()
                        ? {
                            images: [
                                walletList()[3].image,
                                walletList()[4].image,
                ...
                </div>} extraContent={currentInfo() !== 'top-wallets' || isMobile() ? undefined : (<Wallets disabled={isLoading()} small wallets={walletList().slice(-(walletList().length - 4))} isMobile={isMobile()} onClickWallet={handleClickWallet} onClickOther={handleClickOther}/>)} onClickShowMore={handleClickShowMoreContent} onCloseClick={() => handleCloseAlert()} onBackClick={currentInfo() === 'install' && !isMobile()
                    ? () => setCurrentInfo('top-wallets')
                    : currentInfo() === 'qr'
                        ? () => setCurrentInfo('top-wallets')
