...
configs({ modes: ['md'], directions: ['ltr'] }).forEach(({ title, screenshot, config }) => {
  test.describe(title('input: label slot'), () => {
    ...
    test('should render the notch correctly with a slotted label after the input was originally hidden', async ({ page, }) => {
      await page.setContent(`
        <style>
          .custom-label {
            font-size: 30px;
      ...
      `, config);
      const input = page.locator('ion-input');
      await input.evaluate((el) => el.style.removeProperty('display'));
