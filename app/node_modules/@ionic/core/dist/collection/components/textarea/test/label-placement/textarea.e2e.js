...
configs().forEach(({ title, screenshot, config }) => {
  ...
  test.describe(title('textarea: label placement floating'), () => {
    ...
    test('label should appear on top of the textarea and hide the textarea when there is a no value', async ({ page, }) => {
      await page.setContent(`
    ...
    test('label should appear on top of the textarea when there is a placeholder and no value', async ({ page }) => {
      await page.setContent(`
       ...
       `, config);
      const textarea = page.locator('ion-textarea');
      expect(await textarea.screenshot()).toMatchSnapshot(screenshot(`textarea-placement-floating-no-value-placeholder`));
    });
    test('label should appear on top of the textarea when the textarea is focused', async ({ page }) => {
      ...
      expect(await textarea.screenshot({ animations: 'disabled' })).toMatchSnapshot(screenshot(`textarea-focused-placement-floating-no-value`));
    });
    test('textarea should render multiple lines of text', async ({ page }) => {
      await page.setContent(`
