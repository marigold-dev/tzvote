  ...
  test.describe(title('textarea: hint text'), () => {
    test.describe('textarea: hint text functionality', () => {
      test('helper text should be visible initially', async ({ page }) => {
        await page.setContent(`<ion-textarea helper-text="my helper" error-text="my error" label="my textarea"></ion-textarea>`, config);
      ...
      test('error text should be visible when textarea is invalid', async ({ page }) => {
        await page.setContent(`<ion-textarea class="ion-invalid ion-touched" helper-text="my helper" error-text="my error" label="my textarea"></ion-textarea>`, config);
        const helperText = page.locator('ion-textarea .helper-text');
        const errorText = page.locator('ion-textarea .error-text');
    ...
    test.describe('textarea: hint text rendering', () => {
      test.describe('regular textareas', () => {
        ...
        test('should not have visual regressions when rendering error text', async ({ page }) => {
          await page.setContent(`<ion-textarea class="ion-invalid ion-touched" error-text="my helper" label="my textarea"></ion-textarea>`, config);
          const bottomEl = page.locator('ion-textarea .textarea-bottom');
    ...
    test.describe('textarea: counter rendering', () => {
      test.describe('regular textareas', () => {
        test('should not have visual regressions when rendering counter', async ({ page }) => {
          await page.setContent(`<ion-textarea counter="true" maxlength="20" label="my textarea"></ion-textarea>`, config);
          const bottomEl = page.locator('ion-textarea .textarea-bottom');
        ...
        test('should not have visual regressions when rendering counter with helper text', async ({ page }) => {
          await page.setContent(`<ion-textarea label="my textarea" counter="true" maxlength="20" helper-text="my helper"></ion-textarea>`, config);
          const bottomEl = page.locator('ion-textarea .textarea-bottom');
          expect(await bottomEl.screenshot()).toMatchSnapshot(screenshot(`textarea-bottom-content-counter-helper-text`));
        });
        test('should not have visual regressions when rendering counter with error text', async ({ page }) => {
          await page.setContent(`<ion-textarea class="ion-invalid ion-touched" label="my textarea" counter="true" maxlength="20" error-text="my error"></ion-textarea>`, config);
          const bottomEl = page.locator('ion-textarea .textarea-bottom');
