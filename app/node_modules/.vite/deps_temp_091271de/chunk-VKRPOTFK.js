...
function assertArgs$1(ex, n) {
  var _a, _b;
  if (n === 0 && ex.args === void 0 || ((_a = ex.args) === null || _a === void 0 ? void 0 : _a.length) === n) {
    return true;
  }
...
var Parser = class {
  ...
  expand(ex) {
    var _a, _b, _c;
    if (((_a = this.opt) === null || _a === void 0 ? void 0 : _a.expandGlobalConstant) !== void 0 && ex.prim === "constant") {
...
var Formatter = class _Formatter {
  ...
  indent(n = 0) {
    var _a;
    let ret = "";
    if (((_a = this.opt) === null || _a === void 0 ? void 0 : _a.indent) !== void 0) {
  ...
  down(n) {
    return new _Formatter(this.opt, this.lev + n);
  }
};
function hasArgs(node) {
  return "prim" in node && (node.annots !== void 0 && node.annots.length !== 0 || node.args !== void 0 && node.args.length !== 0);
}
...
function emitExpr(node, f, foldMacros) {
  ...
  if (Array.isArray(node)) {
    ...
    if ((node.annots === void 0 || node.annots.length === 0) && (node.args === void 0 || node.args.length === 0)) {
      return node.prim;
    }
    let ret = "(" + node.prim;
    if (node.annots !== void 0) {
...
var TezosIdEncodeError = class extends ParameterValidationError {
  constructor(message) {
    super();
    this.message = message;
...
function writeExpr(expr, wr, tf) {
  ...
  const tag = (((_a = e.args) === null || _a === void 0 ? void 0 : _a.length) || 0) < 3 ? Tag.Prim0 + (((_b = e.args) === null || _b === void 0 ? void 0 : _b.length) || 0) * 2 + (e.annots === void 0 || e.annots.length === 0 ? 0 : 1) : Tag.Prim;
  wr.writeUint8(tag);
  wr.writeUint8(prim);
  if (e.args !== void 0) {
    if (e.args.length < 3) {
  ...
  switch (tag) {
    case Tag.Int: {
      ...
      for (let i = buf.length - 1; i >= 0; i--) {
        const bits = i === 0 ? BigInt(6) : BigInt(7);
        const byte2 = BigInt(buf[i]);
        val <<= bits;
        val |= byte2 & (BigInt(1) << bits) - BigInt(1);
        if (i === 0) {
          sign = !!(byte2 & BigInt(64));
        }
      }
      if (sign) {
        val = -val;
    ...
    case "timestamp":
      return [
        ...
        (d) => {
          ...
          const date = new Date(parseInt(d.int, 10) * 1e3);
...
function unpackData(src, t) {
  ...
  const ex = readExpr(r, t !== void 0 ? getReadTransformFuncs(t) : readPassThrough);
...
function functionTypeInternal(inst, stack, ctx) {
  ...
  function annotateField(arg, field, insAnn, n, defField) {
    var _a, _b, _c, _d;
    const fieldAnn = (_a = argAnn(field).f) === null || _a === void 0 ? void 0 : _a[0];
    ...
    switch (instruction2.prim) {
      ...
      case "PAIR": {
        ...
        const retArgs = s.map((v, i) => {
          var _a2;
          const va = argAnn(v);
          const f = ia.f && ia.f.length > i && ia.f[i] !== "%" ? ia.f[i] === "%@" ? va.v ? ["%" + trim(((_a2 = va.v) === null || _a2 === void 0 ? void 0 : _a2[0]) || "")] : void 0 : [ia.f[i]] : void 0;
      ...
      case "GET_AND_UPDATE": {
        ...
        const va = (_a = ia.v) === null || _a === void 0 ? void 0 : _a.map((v) => v !== "@" ? [v] : void 0);
        if (s[2].prim === "map") {
          return [
...
function traceDumpFunc(blocks, cb) {
  return (v) => {
    ...
    const msg = `${macro ? "Macro" : "Op"}: ${macro ? emitMicheline(macro, void 0, true) + " / " : ""}${emitMicheline(v.op)}
Input:
${formatStack(v.in)}
Output:
${formatStack(v.out)}
...
function formatError(err) {
  ...
  if (err instanceof MichelsonInstructionError) {
    ...
    return `${macro ? "Macro" : "Op"}: ${macro ? emitMicheline(macro, void 0, true) + " / " : ""}${emitMicheline(err.val)}
Stack:
...
//# sourceMappingURL=chunk-VKRPOTFK.js.map
