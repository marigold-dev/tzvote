...
export function isJsonMimeType(mime) {
    return contentTypes[mime] === "json" || /\bjson\b/i.test(mime);
}
export function getBodyFormatter(body) {
    if (body?.content) {
 ...
 */
export function getFormatter({ style = "form", explode = true, content, }) {
    if (content) {
...
export function getOperationIdentifier(id) {
    if (!id)
        return;
    if (id.match(/[^\w\s]/))
        return;
 ...
 */
export function getOperationName(verb, path, operationId) {
 ...
 */
export function refPathToPropertyPath(ref) {
    if (!ref.startsWith("#/")) {
 ...
 */
export function getReferenceName(obj) {
    if (isReference(obj)) {
        return getRefBasename(obj.$ref);
    }
}
export function toIdentifier(s, upperFirst = false) {
 ...
 */
export function createUrlExpression(path, qs) {
    const spans = [];
    ...
    const head = path.replace(/(.*?)\{(.+?)\}(.*?)(?=\{|$)/g, (_substr, head, name, literal) => {
        ...
        spans.push({
            expression: cg.createCall(factory.createIdentifier("encodeURIComponent"), { args: [factory.createIdentifier(expression)] }),
            literal,
        });
        return head;
    });
 ...
 */
export function callQsFunction(name, args) {
 ...
 */
export function callQueryParamParser(name, args) {
    return callExternalFunction("QueryParamsParsers", name, args);
}
function callExternalFunction(namespace, name, args) {
    return cg.createCall(factory.createPropertyAccessExpression(factory.createIdentifier(namespace), name), { args });
}
/**
 * Create a call expression for one of the oazapfts runtime functions.
 */
export function callOazapftsFunction(name, args, typeArgs) {
    return cg.createCall(factory.createPropertyAccessExpression(factory.createIdentifier("oazapfts"), name), { args, typeArgs });
}
/**
 ...
 */
export function supportDeepObjects(params) {
    const res = [];
    const merged = {};
    params.forEach((p) => {
        const m = /^(.+?)\[(.*?)\]/.exec(p.name);
    ...
    */
    getTrueEnum(schema, propName) {
        ...
        const members = values.map((s, index) => {
            ...
            return factory.createEnumMember(factory.createIdentifier(s), schema.type === "number"
                ? factory.createNumericLiteral(index)
                : factory.createStringLiteral(s));
        });
        this.enumAliases.push(factory.createEnumDeclaration([cg.modifier.export], name, members));
    ...
    getTypeFromResponses(responses) {
        return factory.createUnionTypeNode(Object.entries(responses).map(([code, res]) => {
    ...
    addQueryStringParserExtensionsImport = (src) => {
        ...
        const extensionsImport = factory.createImportDeclaration(undefined, factory.createImportClause(false, factory.createIdentifier("QueryParamsParsers"), undefined), factory.createStringLiteral("./queryParamParsers"));
    ...
    generateApi() {
        ...
        const stub = cg.parseFile(path.resolve(__dirname, "../../src/codegen/ApiStub.ts"));
        ...
        cg.changePropertyValue(initializer, "baseUrl", defaultBaseUrl(this.spec.servers || []));
            ...
            Object.keys(this.resolve(item)).forEach((verb) => {
                ...
                if (optional.length) {
                    methodParams.push(cg.createParameter(cg.createObjectBinding(optional
                        ...
                        type: factory.createTypeLiteralNode(optional.map((p) => cg.createPropertySignature({
                ...
                if (query.length) {
                    ...
                    qs = callQsFunction("query", Object.entries(paramsByFormatter).map(([format, params]) => {
                        ...
                        const qsCallParamsObjectLiteral = factory.updateObjectLiteralExpression(basicParamsObjLiteral, [
                ...
                if (header.length) {
                    init.push(factory.createPropertyAssignment("headers", factory.createObjectLiteralExpression([
                        factory.createSpreadAssignment(factory.createLogicalAnd(factory.createIdentifier("opts"), factory.createPropertyAccessExpression(factory.createIdentifier("opts"), "headers"))),
                        ...header.map((param) => cg.createPropertyAssignment(param.name, factory.createIdentifier(getArgName(param)))),
                    ], true)));
                }
                const args = [url];
                if (init.length) {
                ...
                functions.push(cg.addComment(cg.createFunctionDeclaration(name, {
                    modifiers: [cg.modifier.export],
                }, methodParams, cg.block(factory.createReturnStatement(this.wrapResult(callOazapftsFunction({
                    json: "fetchJson",
                    text: "fetchText",
                    blob: "fetchBlob",
